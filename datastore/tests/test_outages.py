...
def one_system_outage(cursor):
    ...
    cursor.execute(
        ...
        (some_uuid, start.strftime('%Y-%m-%d %H:%M'), end.strftime('%Y-%m-%d %H:%M'))
    )
    return some_uuid
...
def test_delete_report_outage(cursor, valueset, new_report, add_perm):
    ...
    cursor.execute(
        "INSERT INTO report_outages (id, report_id, start, end) VALUES (%s, %s, %s, %s)",
        (report_id, some_uuid,
         start.strftime('%Y-%m-%d %H:%M'),
         end.strftime('%Y-%m-%d %H:%M'))
    )
    cursor.execute("SELECT * FROM report_outages WHERE report_id = %s", (report_id,))
